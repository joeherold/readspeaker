 ...
 * @package    Devtools
 */
class CE_ReadSpeakerDoc extends \ContentElement {

    /**
     * Template
     * @var string
     */
    protected $strTemplate = 'ce_readspeaker_doc';
    ...
    public function generate() {
        ...
        if (TL_MODE == 'BE') {
            ...
            $icon = TL_ASSETS_URL . 'assets/contao/images/' . $file->icon;
            ...
            $objTemplate->wildcard .= '<img style="padding: 0 10px;height:20px; float: right;" src="system/modules/readspeaker/assets/ReadSpeaker_logo.png" >';


            $objTemplate->wildcard .= sprintf('
                                            <div class="rs_player_container block>
                                                <a href="' . $url . '" target="_blank"><img src="' . $icon . '" > ' . $fileObj->name . ' (' . $this->getReadableSize($file->filesize, 1) . ')</a>
                                                
                                                <img src="system/modules/readspeaker/assets/appicon.png" style="border-style: none; vertical-align: text-bottom;" alt="ReadSpeaker"> ' . $this->rs_player_title . '</a>
                                            </div>

                                        ', $this->rs_player_title . ($row['docreader_id'] ? '' : ' <em>(DEMO)</em>'));



            $objTemplate->wildcard .= '<div style="padding:2px 10px 0px 0px;">';
            if ($row) {
                ...
                if ($row['have_license'] && !$row['docreader_id']) {
                   $objTemplate->wildcard .= '<p class="tl_error">docReader ID missing in
                       ' . $GLOBALS['TL_LANG']['MSC']['rs_language_codes'][$row['language']] . ' ' . ($row['domain'] ? '<span class="tl_green">[' . $row['domain'] . ']</span>' : '') . ' ' . ($row['note'] ? '<span class="tl_blue" >[' . $row['note'] . ']</span>' : '') . '</p>';
                }
